...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # # 합격 여자
        # SendStar.objects.create(user=man4, partner=hjk, star=5)
        # SendStar.objects.create(user=man5, partner=hjk, star=3)
        # SendStar.objects.create(user=man6, partner=hjk, star=4)
        # # 심사중 여자
        # SendStar.objects.create(user=man4, partner=szj, star=2)
        # SendStar.objects.create(user=man5, partner=szj, star=3)
        # # 합격 남자
        # SendStar.objects.create(user=man1, partner=hgo, star=5)
        # SendStar.objects.create(user=man2, partner=hgo, star=3)
        # SendStar.objects.create(user=man3, partner=hgo, star=2)
        # # 불합격 남자
        # SendStar.objects.create(user=man1, partner=hbb, star=2)
        # SendStar.objects.create(user=man2, partner=hbb, star=4)
        # SendStar.objects.create(user=man3, partner=hbb, star=1)
        # # 심사중 남자
        # SendStar.objects.create(user=man2, partner=dhl, star=3)
        # print('가입심사가 이루어졌습니다.')
